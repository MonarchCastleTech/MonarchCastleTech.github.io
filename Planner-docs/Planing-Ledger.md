...
- 2026-09-23: SEO/GEO/GenAI wave-2 — pageFaqs per narrative route + renderPageFaq `#faq`; pageExtraJsonLd (SoftwareApplication/ItemList/OfferCatalog/CollectionPage+Dataset/Article/Blog/AboutPage/Service); faqJsonLd generalized; head rel=agent+logo preload; robots aiBots expanded; llms Glossary+answers pointer; llms-full FAQ+Glossary; ai/agents `#answers`+pricing; static/404.html noindex recovery; worker LINK agent/manifest/alternate/author; tests extended; test 71/71 + test:dist 13/13 + verify-dist pass; edge NS still Squarespace
